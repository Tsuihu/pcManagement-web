<template>
  <div class="collectors">
    <el-tag>位置 <i class="el-icon-arrow-right"></i> 管理人员列表</el-tag>
    <!-- 查询，添加 -->
...
export default {
  ...
  methods: {
    ...
    // 添加dialog
    addOnSubmit() {
      api.post('/manager/registe.do',this.addFormData).then(res => {
        if(res.code == this.$comm.RESULT_CODE.SUCCESS) {
          // console.log(res)
    ...
    editOnSubmit() {
      ...
      api.post('/manager/updateManager.do',this.EditFormData).then(res => {
        if(res.code == this.$comm.RESULT_CODE.SUCCESS) {
          // console.log(res)
